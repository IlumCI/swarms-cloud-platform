...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!force && cached && cached.expiresAt > now) {
      return NextResponse.json(cached.data, {
        headers: {
          ...
          'Cache-Control': `private, max-age=${TEN_HOURS_SECONDS}`,
        },
      });
    }

    const client = new SwarmsAPIClient(apiKey, process.env.SWARMS_API_BASE_URL);
    const data = await client.getAvailableSwarmTypes();
    ...
    console.error('Error fetching swarm types:', error);
    return NextResponse.json(
      {
        error: error.message || 'Failed to fetch swarm types',
